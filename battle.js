// ======================================
// Star Signal - battle.js 完全版（コンパクトUI対応）
// 前提となる主な要素:
//   #enemy-area
//   #party-area
//   #command-area
//   #log
//
// 既存HTMLにボタンがなくても command-area 内に自動生成します。
// 既存HTMLで inline onclick="selectAction('attack')" を使っていても動くように
// window.selectAction を公開しています。
// ======================================
...
(function () {
  ...
  const party = [
    {
      ...
      specialReady: false,
      skills: [
        { name: '疾風斬', type: 'damage', power: 1.35, gaugeGain: 10, description: '単体に素早い斬撃' },
        { name: '勇気の号令', type: 'buff', gaugeGain: 5, description: '次の通常攻撃を強化', effect: 'atkUp' }
      ]
    },
    {
      id: 'member1',
      ...
      skills: [
        ...
        { name: 'ライトアロー', type: 'damage', power: 1.2, gaugeGain: 8, description: '単体に光属性攻撃' }
      ]
    },
    {
      id: 'member2',
      ...
      specialReady: false,
      skills: [
        { name: '鋼の一撃', type: 'damage', power: 1.15, gaugeGain: 10, description: '単体攻撃＋自己回復', selfHeal: 15 },
        { name: '挑発の気配', type: 'buff', gaugeGain: 5, description: '次の被ダメージを軽減', effect: 'guardUp' }
      ]
    },
    {
      id: 'member3',
      ...
      specialReady: false,
      skills: [
        { name: 'フレイムバースト', type: 'damage', power: 1.25, gaugeGain: 10, description: '炎の単体魔法' },
        { name: 'サンダーフォール', type: 'damage', power: 1.5, gaugeGain: 12, description: '高火力の雷撃' }
      ]
    }
  ];
  ...
  const ui = {
    buttonsRow: document.getElementById('battle-buttons-row'),
  ...
  function memberPercentHp(member) {
    return clamp((member.hp / member.maxHp) * 100, 0, 100);
  }

  function memberPercentGauge(member) {
    return clamp((member.gauge / 100) * 100, 0, 100);
  }

  function enemyPercentHp() {
    return clamp((enemy.hp / enemy.maxHp) * 100, 0, 100);
  }
  ...
  function gainGauge(member, amount) {
    member.gauge = clamp(member.gauge + amount, 0, 100);
  }
  ...
  function renderEnemy() {
    els.enemyArea.innerHTML = `
      <div class="enemy-wrap">
        ...
        <div class="enemy-hp-text">${Math.max(0, Math.ceil(enemy.hp))} / ${enemy.maxHp}</div>
      </div>
    `;
  }
  ...
  function renderParty() {
    ...
    party.forEach((member, index) => {
      ...
      // ✅ 採用した active 表示
      if (state.phase === 'player' && index === state.actorIndex && member.alive) {
      ...
      card.innerHTML = `
        ...
        <div class="hp-bar">
          <div class="hp-fill" style="width:${memberPercentHp(member)}%"></div>
        </div>
        <div class="mini-text">HP ${Math.max(0, Math.ceil(member.hp))}/${member.maxHp}</div>
        <div class="gauge-bar">
          <div class="gauge-fill" style="width:${memberPercentGauge(member)}%"></div>
        </div>
  ...
  function useSkill(skillIndex) {
    ...
    if (skill.type === 'damage') {
      ...
      enemy.hp -= damage;
      appendLog(`${actor.name}のスキル「${skill.name}」！ ${enemy.name}に${damage}ダメージ。`);
    ...
    if (skill.type === 'heal') {
      ...
      if (target) {
        ...
        appendLog(`${actor.name}のスキル「${skill.name}」！ ${target.name}のHPが${skill.heal}回復。`);
      }
    }
    ...
    if (skill.type === 'buff') {
      if (skill.effect === 'atkUp') {
        ...
        appendLog(`${actor.name}のスキル「${skill.name}」！ 次の通常攻撃が強化された。`);
      }
      if (skill.effect === 'guardUp') {
        actor.tempGuardBoost = true;
        appendLog(`${actor.name}のスキル「${skill.name}」！ 次の被ダメージを軽減する。`);
      }
    }
  ...
  function defend() {
    ...
    actor.guarding = true;
    actor.def = actor.baseDef * 5; // ✅ 指定どおり DEF 5倍
    gainGauge(actor, 10);
  ...
  function enemyTurn() {
    ...
    if (target.tempGuardBoost) {
      targetDef = Math.round(targetDef * 1.35);
    }
  ...
  function returnToStory() {
    try {
      const resultPayload = {
        result: state.result,
        nextScene: battleContext && typeof battleContext.nextScene === 'number'
          ? battleContext.nextScene
          : null
      };
